...
- NEVER "EXCLUDES OUTRIGHT": no single bedside test excludes a can't-miss diagnosis absolutely. Say "makes X very unlikely" or "effectively rules out in this context", and where the reference notes limits (a negative urine pregnancy test very early in pregnancy; ketosis with near-normal glucose) keep one short line of residual caution. Reserve the word "excluded" for the reference's own hard criteria. This applies to terse worklist lines too: never label a step "— excludes X"; write "— closes the X branch if negative" or "— makes X very unlikely".
